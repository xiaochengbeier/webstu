...
## 二叉平衡预构建，搜索二叉先出现。
    构建平衡二叉树是对二叉搜索树的优化减少二叉搜索树的深度
## 参数检查先构建，是否平衡返回现。
    首先检查传递入得 root 是不是平衡二叉树如果是那么就返回root
## 倒序遍历顺序建，左右不空平衡现。
    对于二叉树我们需要使用后续遍历的方式构建
    若果当前节点的左子树不为空那么就把左子树构建为平衡二叉树
    右子树同理
## 获得深度攀比建，左右比较旋转现。
    然后获得左右子树的深度判断是左旋还是右旋
    如果左边深那么就右旋
    如果右边深那么就左旋
## 旋转左右右左建，变化分支深度现。
    但是单单是但旋还不足以将一棵树构建为平衡二叉树
    判断如果说新根的变化分支比不变深度深的话那么需要进行右左旋转 或者左右旋转
    如果是在左单旋下 那么就进行右左双旋
    如果是在右单旋下 那么就进行左右双旋
## 然后单旋再构建，获得新根节点现。
## 左左右右还构建，平衡由此终相见。
    然后单旋转构建完毕后还需要将新的根节点进行左左双旋或者右右双旋
```js
      class Node {
            constructor(value) {
                this.value = value;
